// THIS PLUGIN IS ONLY FOR EM32 based devices
...
const char *progname = "check_em32";
const char *revision = "$Revision: 2.1 $";
const char *copyright = "2015";
const char *email = "support@eEsensors.com";
	...
	case 'G':  //Cacti/RRDTool Output TempF:** Humid:**
		{
			    ...
			    if(strcmp(shtStatus,"inline") == 0){//SHT Sensor is present/ enabled. Get readings.
					...
					//Get Humidity Data
					const char *p3 = strstr(iobuf, "<hu0>")+5;//adding 5 to indicate length of <tm0>
				    const char *p4 = strstr(iobuf, "</hu0>");
				    len = p4-p3;
				    char *shtHum = (char*)malloc(sizeof(char)*(len+1));
				    strncpy(shtHum, p3, len);
				    shtHum[len] = '\0';

					//Get Temperature Unit
				    const char *p5 = strstr(iobuf, "<tun0>")+6;
				    const char *p6 = strstr(iobuf, "</tun0>");
				    len = p6-p5;
				    char *tempUnit = (char*)malloc(sizeof(char)*(len+1));
				    strncpy(tempUnit, p5, len);
				    tempUnit[len] = '\0';
